...
data class PurchaseTypeRequest(@field:NotBlank val name: String)
data class PurchaseTypeResponse(val id: Long, val name: String)
...
class PurchaseTypeController(
    ...
    @GetMapping
    fun list(): List<PurchaseTypeResponse> =
        purchaseTypeRepository.findAll().map { it.toResponse() }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    fun create(@Valid @RequestBody req: PurchaseTypeRequest): PurchaseTypeResponse =
        purchaseTypeRepository.save(PurchaseType(name = req.name)).toResponse()

    @PutMapping("/{id}")
    fun update(@PathVariable id: Long, @Valid @RequestBody req: PurchaseTypeRequest): PurchaseTypeResponse {
        val existing = purchaseTypeRepository.findById(id)
